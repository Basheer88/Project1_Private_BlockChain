...
class Blockchain {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTime - time < 300 ) {
                if(bitcoinMessage.verify(message, address, signature)){
                    ...
                    reject(Error("Verification Faile"));
                }
            }
            else {
                reject(Error("Not Added"));
            }     
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            const result = self.chain.filter(bl => bl.hash==hash);
            if(result!=null){
                resolve(result);
            }
            else{
                reject(Error('No Block with hash: ' + hash));
            }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for(let i=0; i<self.chain.length; i++){
                ...
                if(i>0){
                   if(self.chain[i-1].hash !== self.chain[i].hash){
                        errorLog.push({error: 'Previous block hash doesnt match', block: self.chain[i]});
                   }
                }
            }
            resolve (errorLog);
